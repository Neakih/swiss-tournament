...
void print_result(player arr[])
{
	...
	for(int j = 0; j < N; j ++)
	{
		printf("%d位, 背番号%d, ",j + 1, arr[j].sebangou);
		for(int i = 0; i < cr_round; i ++)
		{
			printf("%d",arr[j].kekka[i].aite);
			printf("(%c),", arr[j].kekka[i].WL == 1 ? 'W' : 'L');
		}
		printf("\n");
	}
